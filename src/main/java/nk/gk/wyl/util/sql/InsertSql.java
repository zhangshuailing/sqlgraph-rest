package nk.gk.wyl.util.sql;

import nk.gk.wyl.util.CommonValidator;
import nk.gk.wyl.util.DataUtil;
...
public class InsertSql {
    ...
    public static Map<String, String> getFieldsValuesSqlExact(Map<String, Object> map) {
        ...
        for (Map.Entry<String, Object> key : map.entrySet()) {
            ...
            String type = DataUtil.getType(obj);
            if (StringUtils.isEmpty(fields_sql)) {// 判断是否是空
                ...
                if ("String".equals(type)) {// 字符串时插入数据
                    ...
                    List list  = (List) obj;
                    values_sql = "" + list.toString().replaceAll("\"","'")  + "";
                } else {
                    values_sql = "" + obj + "";
                }
            } else {
                ...
                if ("String".equals(type)){// 字符串时插入数据
                    ...
                    List list  = (List) obj;
                    values_sql = values_sql + "," + list.toString().replaceAll("\"","'") + "";
                } else {
